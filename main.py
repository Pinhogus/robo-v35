...
def rankear(jogos):
    ...
    top_under = sorted([j for j in jogos if j["score_under"] > 0], key=lambda x: x["score_under"], reverse=True)[:10]
    top_over = sorted([j for j in jogos if j["score_over"] > 0], key=lambda x: x["score_over"], reverse=True)[:10]
...
def montar_mensagem(top_under, top_over):
    ...
    if top_over:
        ...
        msg += "Nenhum qualificado.\n"
    msg += "\n--------------------\nUse como referencia, nao como garantia."
    return msg
